...
class MyBot:
    def __init__(self):
        ...
        self.dp.register_message_handler(self.cmd_close_reset,         commands = "reset")
    ...
    async def cmd_close_reset(self, message: types.Message):
        await self.cmd_open_close(message = message, status = 0, reset_vote = 1)
    ...
    async def cmd_open_close(self, message: types.Message, status, reset_vote = 0):
        chat_id = message.chat.id
        msg_id_del = message.message_id
        user_id = message.from_user.id
