...
1.a)  /game game_cost|max_players|last_games|заголовок|кнопка_да|кнопка_нет|кнопка_?|кнопка_+|кнопка_-
  b)  /single заголовок|кнопка_1|...|кнопка_N
  c)  /multi заголовок|кнопка_1|...|кнопка_N
  d)  /game2 game_cost|max_players|last_games|заголовок|кнопка_да|кнопка_нет|кнопка_?|кнопка_+|кнопка_-|дополнительная_кнопка
2.    /get status*
3.a)  /close msg_id*
  b)  /closeall
  c)  /reset msg_id*
...
16.a) /statall last_games|msg_id**
   b) /statuser user_id|last_games|msg_id**
   c) /statover
...
3. Закрыть опрос/опросы.

    a) Закрыть один опрос.
        Команда:
          /close msg_id*
    ...
    b) Закрыть все опросы.
        Команда:
          /closeall

        Бот закроет и открепит все опросы в группе.

    c) Закрыть опрос со сбросом нажатий кнопки "кнопка_да".
        Команда:
          /reset msg_id*
        ...
        Бот закроет и открепит опрос в группе. При этом все нажатия на кнопку "кнопка_да" заменяются на нажатие кнопки "кнопка_нет". В случае, если твой открытый опрос в группе только один, msg_id указывать необязательно.    
...
16. Получить статистику.
  
  a) Получить статистику по игрокам.
    Команда:
      /statall last_games|msg_id*
  ...
  b) Получить статистику по одному игроку.
    Команда:
      /statuser user_id|last_games|msg_id*
    ...
    Бот вернёт в личном сообщении количество участий игрока в кворуме за предыдущие last_games опросов. В статистике участвуют опросы вида 1.a) и 1.d). Статистика отсчитывается за прошлые last_games опросов от:
      - от текущего момента, если msg_id не указан;
      - от даты опроса, указанного в msg_id.

  c) Получить статистику переполнения кворума.
    Команда:
      /statover

    Бот вернёт в личном сообщении список опросов, в которых возникало переполнение кворума. В статистике участвуют опросы вида 1.a) и 1.d).
